...
class SeismicCatalog():
    ...
    def calculate_latlon_coordinates(self):
        ...
        if self.N:
            ...
            try:
                self.latitude, self.longitude = utm.to_latlon(self.easting,
                                                              self.northing,
                                                              zone_id,
                                                              zone_letter)
    ...
    def calculate_seismic_characteristics(self, magnitude_bin_res=0.1, time_bin_segments=10, time_bin_overlap=0.0):
        """
        ...
        if M:
            ...
            bins_left, bins_right = get_overlapping_bin_ids(M, time_bin_segments, time_bin_overlap)
            ...
            for ii in range(time_bin_segments):
                ...
                tmp = gutenberg_richter_a_b(magnitude_slice[bins_left[ii]:bins_right[ii]], magnitude_bins, dt)
                self.varying_b_value[ii] = tmp[1]

            # Estimate magnitude rate as a function of time
            time_bins = np.linspace(self.time_range[0],
                                    self.time_range[-1],
        ...
        self.logger.debug('Estimated magnitude of completeness = %1.2f' % (self.magnitude_completeness))
    ...
    def get_copy(self,
        ...
        self.set_slice(time_range=time_range,
        ...
        self.set_slice(time_range=old_time_range,
                       magnitude_range=old_magnitude_range)
